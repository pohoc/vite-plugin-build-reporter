...
test("collectAssets collects all entries", () => {
  const records = collectAssets(sampleBundle, { gzip: true, brotli: true });
...
test("collectAssets detects asset types", () => {
  const records = collectAssets(sampleBundle, { gzip: true, brotli: true });
...
test("collectAssets computes raw size from code/source", () => {
  const records = collectAssets(sampleBundle, { gzip: true, brotli: true });
...
test("collectAssets computes gzip and brotli smaller than raw", () => {
  const records = collectAssets(sampleBundle, { gzip: true, brotli: true });
  const js = records.find((r) => r.name === "static/js/app-abc.js");
...
test("collectAssets marks disabled compression metrics as unmeasured", () => {
  const records = collectAssets(sampleBundle, { gzip: false, brotli: false });
  const js = records.find((r) => r.name === "static/js/app-abc.js");
  expect(js?.gzip).toBeNull();
  expect(js?.brotli).toBeNull();
});

test("collectAssets excludes source maps by default", () => {
  const bundle: Record<string, Collectable> = {
    "app.js": { type: "chunk", code: "console.log(1)" },
    "app.js.map": { type: "asset", source: '{"version":3}' },
  };
  expect(collectAssets(bundle, { gzip: false, brotli: false }).map((item) => item.name)).toEqual([
    "app.js",
  ]);
  expect(
    collectAssets(bundle, { gzip: false, brotli: false, includeSourceMaps: true }).map(
      (item) => item.name,
    ),
